document.addEventListener('DOMContentLoaded', function() {
  ...
  form.addEventListener("submit", function(event) {
      ...
      log.textContent = `Searching for ${mediaType}: "${query}"`;
  ...
  function displayResults(results) {
      ...
      document.querySelectorAll('.select-btn').forEach(button => {
          button.addEventListener('click', function() {
              ...
              log.textContent = `Generating recommendations...`;
  ...
  function displayRecommendations(selectedItem, recommendations) {
      ...
      recommendations.forEach(rec => {
          ...
          card.innerHTML = `
              <div class="similarity-badge">${rec.similarity_score}% match</div>
              <img src="${item.image_url}" alt="${item.title}">
              <div class="rec-info">
                  <h3>${item.title}</h3>
      ...
      document.querySelectorAll('.recommendation-card .select-btn').forEach(button => {
          button.addEventListener('click', function() {
              const selectedId = this.getAttribute('data-id');
              log.textContent = `Generating new recommendations...`;
